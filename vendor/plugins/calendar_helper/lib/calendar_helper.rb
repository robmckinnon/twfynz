...
module CalendarHelper
  ...
  def calendar(options = {}, &block)
    ...
    heading_prefix = options[:heading_prefix].to_s

    cal  = %(<table class="#{table_class}" border="0" cellspacing="0" cellpadding="0">)
    cal << %(<thead>)
    cal << %(  <tr class="#{month_name_class}">)
    cal << %(    <th colspan="7">#{heading_prefix}#{month_name[0,3]} #{options[:year]}</th>)
    cal << %(  </tr>)
    cal << %(  <tr class="#{day_name_class}">)
    day_names.each do |name|
      unless weekend?(name)
        cal << "    <th>#{name[options[:abbrev]]}</th>"
      end
    end
    ...
    first_day_in_month = Date.civil(options[:year], options[:month], 1)
    last_day_in_month = Date.civil(options[:year], options[:month], -1)

    first_day_is_a_saturday = (options[:first_day_of_week] == 0 and first_day_in_month.wday == 6)

    unless first_day_is_a_saturday
      beginning_of_week(first_day_in_month, first_weekday).upto(first_day_in_month - 1) do |d|
        unless weekend?(d)
          cal << %(<td class="#{options[:other_month_class]})
          cal << " weekendDay" if weekend?(d)
          cal << %(">#{d.day}</td>)
        end
      end unless first_day_in_month.wday == first_weekday
    end

    first_day_in_month.upto(last_day_in_month) do |cur|
    ...
    last_day_is_a_sunday = (options[:first_day_of_week] == 0 and last_day_in_month.wday == 0)

    unless last_day_is_a_sunday
      (last_day_in_month + 1).upto(beginning_of_week(last_day_in_month + 7, first_weekday) - 1)  do |d|
        unless weekend?(d)
          cal << %(<td class="#{options[:other_month_class]})
          cal << " weekendDay" if weekend?(d)
          cal << %(">#{d.day}</td>)
        end
      end unless last_day_in_month.wday == last_weekday
    end
    cal << "</tr></tbody></table>"
  end
  ...
  private
    ...
    def weekend?(date)
      if date.is_a? String
        (date.include?('Sun') or date.include?('Sat'))
      else

class Tracking < ActiveRecord::Base
  ...
  def self.from_user_and_item user, item
    if (item and user)
      item_type = get_item_type item
      Tracking.find_by_user_id_and_item_type_and_item_id user.id, item_type, item.id
    else
      nil
    end
  end

  def self.all_for_item item, user=nil
    item_type = get_item_type item
    trackings = Tracking.find_all_by_item_id_and_item_type item.id, item_type
    if user
